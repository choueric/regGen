...
#define REG_PLL_CONTROL 0x0 // 0
	#define REG_PLL_CONTROL_BYPASS_BIT BIT(1)
	#define REG_PLL_CONTROL_BYPASS_POS 1
	#define REG_PLL_CONTROL_BYPASS_VAL(rv) (((rv) & BIT(1)) >> 1)
		#define REG_PLL_CONTROL_BYPASS_ENABLE	1	// 0b1	0x1
		#define REG_PLL_CONTROL_BYPASS_DISABLE	0	// 0b0	0x0
	#define REG_PLL_CONTROL_FREE_RUN_STR 6
	#define REG_PLL_CONTROL_FREE_RUN_END 7
	#define REG_PLL_CONTROL_FREE_RUN_MSK MASK(6, 7)
	#define REG_PLL_CONTROL_FREE_RUN_VAL(rv) (((rv) & REG_FREE_RUN_MSK) >> 6)
	#define REG_PLL_CONTROL_FREE_RUN_SFT(v) (((v) & MASK(0, 1)) << 6)

#define REG_PLL_2 0x2 // 2
	#define REG_PLL_2_TYPE_STR 4
	#define REG_PLL_2_TYPE_END 5
	#define REG_PLL_2_TYPE_MSK MASK(4, 5)
	#define REG_PLL_2_TYPE_VAL(rv) (((rv) & REG_TYPE_MSK) >> 4)
	#define REG_PLL_2_TYPE_SFT(v) (((v) & MASK(0, 1)) << 4)
		#define REG_PLL_2_TYPE_CLIENT	0	// 0b0	0x0
		#define REG_PLL_2_TYPE_SERVER	1	// 0b1	0x1
		#define REG_PLL_2_TYPE_ROUTE	2	// 0b10	0x2
		#define REG_PLL_2_TYPE_PEER	3	// 0b11	0x3
...
#define REG_I2C_CLOCK 0x0 // 0
	#define REG_I2C_CLOCK_ENABLE_BIT BIT(0)
	#define REG_I2C_CLOCK_ENABLE_POS 0
	#define REG_I2C_CLOCK_ENABLE_VAL(rv) ((rv) & BIT(0))

#define REG_I2C_DATA 0x1 // 1
	#define REG_I2C_DATA_DATA_STR 0
	#define REG_I2C_DATA_DATA_END 7
	#define REG_I2C_DATA_DATA_MSK MASK(0, 7)
	#define REG_I2C_DATA_DATA_VAL(rv) (rv)
	#define REG_I2C_DATA_DATA_SFT(v) (v)

#define REG_I2C_CONTROL 0x2 // 2
	#define REG_I2C_CONTROL_START_BIT BIT(0)
	#define REG_I2C_CONTROL_START_POS 0
	#define REG_I2C_CONTROL_START_VAL(rv) ((rv) & BIT(0))
		#define REG_I2C_CONTROL_START_START	1	// 0b1	0x1
		#define REG_I2C_CONTROL_START_STOP	0	// 0b0	0x0
	#define REG_I2C_CONTROL_INTERUPT_BIT BIT(1)
	#define REG_I2C_CONTROL_INTERUPT_POS 1
	#define REG_I2C_CONTROL_INTERUPT_VAL(rv) (((rv) & BIT(1)) >> 1)
`
)
...
func TestCmacroComplex(t *testing.T) {
	...
	var longResult bytes.Buffer
	fmtter.FormatRegJar(&longResult, jar, true)
	if strings.Compare(longResult.String(), cmacroCmplxFull) != 0 {
		goutils.PrintStringMismatch(longResult.Bytes(), []byte(cmacroCmplxFull), dbg.True)
		t.Fatal("complex cmarco output does not match!")
	}
}
